...
# if [ -z "$1" ]; then
#     echo "usage: update-versions /PATH/TO/SCF"
#     exit
# fi
...
VERSION_INFO=$(cat "${SCF_ROOT}/_work/VERSION_INFO")

declare -A MISSING_REASONS=(
    [consul-release]="not used"
    [cf-networking-release]="not used"
    [capi-release]="not used"
    [cflinuxfs2-release]="not used"
    [cf-routing-release]="not used"
    [cf-smoke-tests-release]="not used"
    [cf-syslog-drain--release]="not used"
    [diego-release]="not used"
    [garden-runc-release]="not used"
    [loggregator-release]="not used"
    [nats-release]="not used"
    [statsd-injector-release]="not used"
)
